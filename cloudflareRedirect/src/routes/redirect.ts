...
export function parseDestinationFromQuery(url: string): { destination: string; debugMode: boolean } {
  ...
  if (isEncoded) {
    ...
    // This handles cases like: /r?to=https://example.com?var1=abc&var2=def
    destinationRaw = remainingQuery
  }

  // Decode if encoded
  const destination = isEncoded ? decodeURIComponent(destinationRaw) : destinationRaw
  ...
  // Also check for 'debug=' AFTER 'to=' in the outer query (only if encoded)
  if (isEncoded && !debugMode) {
    ...
    if (debugMatchAfter) {
      ...
      // When debug is in outer query, strip query params from destination for cleaner debug output
      const destUrlObj = new URL(destination)
      const pathname = destUrlObj.pathname === '/' ? '' : destUrlObj.pathname
      cleanDestination = `${destUrlObj.protocol}//${destUrlObj.host}${pathname}`
    }
  }
...
app.get('/', async (c) => {
  try {
    ...
    // Hono auto-decodes query params, so toParam is already decoded
    // No need for decodeURIComponent() - removed redundant decoding (AC#9)
    const destination = toParam

    // Check for debug parameter (AC#1: renamed from 'n' to 'debug')
    // Support legacy 'n' parameter with warning for backwards compatibility
    const debugParam = c.req.query('debug') || c.req.query('n')
    const debugMode = debugParam === '1' || debugParam === 'true'

    // Log legacy parameter usage for monitoring (AC#1)
    if (c.req.query('n') && !c.req.query('debug')) {
